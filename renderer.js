const Ipc = require('./src/renderer/ipcClient');
...
const { buildBalanceCsv, buildShareExport } = require('./src/application/exportData');
const { createMonotonicIdGenerator } = require('./src/application/idGenerator');
const JSZip = require('jszip');
const { jsPDF } = require('jspdf');
const html2canvas = require('html2canvas');
const { escapeHtml, safeColor } = require('./src/renderer/html');

const FORMULA_PRESETS = [
    { name: "Standard (Effective HP)", desc: "Defense increases HP as %. Stable scaling.", dmg: "a.atk * (100 / (100 + b.def))", hit: "a.acc - b.eva", cp: "hp * 0.5 + atk * 2 + def * 1.5 + acc + eva + aspd * 5" },
    { name: "Classic (Subtraction)", desc: "ATK minus DEF. Includes min damage check.", dmg: "(a.atk - b.def) > 1 ? (a.atk - b.def) : 1", hit: "a.acc - b.eva", cp: "hp + atk + def + acc + eva + aspd * 10" },
    { name: "Simple Percent", desc: "1 Def = 1% reduction. Needs capping.", dmg: "a.atk * (1 - (b.def / 100))", hit: "a.acc * (1 - (b.eva / 100))", cp: "hp * 0.5 + atk * 1.5 + def * 2 + acc + eva" }
];

const createId = createMonotonicIdGenerator();
...
const ModalSystem = {
    ...
    show(msg, type, onConfirm, onCancel) {
        ...
        this.msgEl.textContent = typeof msg === 'string' ? msg : '';
...
const checkFormula = (formula) => {
    ...
    return Sim.validateFormula(formula, rules && Array.isArray(rules.stats) ? rules.stats : []);
};
...
function showOperationError(result, fallbackMessage) {
    const message = result && result.message ? result.message : fallbackMessage;
    const details = result && result.details ? `\n${result.details}` : '';
    ModalSystem.alert(`${message}${details}`);
}

async function saveProject(saveAs = false) {
    const result = await Ipc.saveProject(DM.getProjectData(), saveAs);
    if (result.status === 'cancelled') return false;
    if (result.status !== 'ok') {
        showOperationError(result, 'Could not save the project.');
        return false;
    }
    if (result.reusedPath) console.log('Project saved to the current file.');
    else ModalSystem.alert(result.message);
    setModified(false);
    return true;
}

async function loadProject() {
    const result = await Ipc.loadProject();
    if (result.status === 'cancelled') return;
    if (result.status !== 'ok') {
        showOperationError(result, 'Could not load the project.');
        return;
    }
    try {
        DM.loadProject(result.data);
    } catch (error) {
        ModalSystem.alert(`Could not load project: ${error.message}`);
        return;
    }
...
function updateComparisonDropdown() {
    ...
    snapshots.forEach((snap, idx) => { const date = new Date(snap.date).toLocaleDateString(); const selected = idx === comparisonSnapshotIndex ? 'selected' : ''; html += `<option value="${idx}" ${selected}>${escapeHtml(snap.name)} (${escapeHtml(date)})</option>`; });
...
async function generateReport() {
    ...
    try {
        ...
        <div style="display:grid; grid-template-columns: 80px repeat(${entities.length}, 1fr); gap:1px; border:1px solid #333;">
            ...
            ${entities.map(e => `<div style="padding:5px; background:#2f3136; color:${safeColor(e.color)}; font-size:11px; font-weight:bold; text-align:center; overflow:hidden;">${escapeHtml(String(e.name).substring(0,6))}</div>`).join('')}
        `;

        for(let i=0; i<entities.length; i++) {
            matrixHTML += `<div style="padding:5px; background:#2f3136; color:${safeColor(entities[i].color)}; font-size:11px; font-weight:bold; display:flex; align-items:center;">${escapeHtml(entities[i].name)}</div>`;
            for(let j=0; j<entities.length; j++) {
        ...
        for (let idx = 0; idx < entities.length; idx++) {
             ...
             <div style="background:#2f3136; border-left: 5px solid ${safeColor(ent.color)}; padding:15px; border-radius:6px; color:#eee; font-family:sans-serif; width:700px; margin-bottom:10px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; border-bottom:1px solid #444; padding-bottom:8px;">
                    <span style="font-size:20px; font-weight:bold;">${escapeHtml(ent.name)}</span>
                ...
                <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:8px;">
                    ${Object.entries(stats).map(([k, v]) => `
                        <div style="background:#202225; padding:6px; border-radius:4px; text-align:center;">
                            <div style="font-size:10px; color:#888;">${escapeHtml(k.toUpperCase())}</div>
...
async function exportToCSV() {
    try {
        const csv = buildBalanceCsv({
            entities: DM.getEntities(), items: DM.getItems(), rules: DM.getRules(),
            maxLevel: parseInt(dom.maxLevel.value) || 20, metric: dom.metric.value,
            getStatsAtLevel: Sim.getStatsAtLevel, calculateValue: Sim.calculateValue
        });
        const result = await Ipc.exportCsv(csv);
        if (result.status === 'ok') ModalSystem.alert(result.message);
        else if (result.status !== 'cancelled') showOperationError(result, 'Could not export CSV.');
    } catch (error) {
        ModalSystem.alert(`Could not export CSV: ${error.message}`);
    }
}
async function exportToJSON() {
    const exportData = buildShareExport({ meta: DM.getMeta(), rules: DM.getRules(), entities: DM.getEntities(), items: DM.getItems() });
    const result = await Ipc.exportJson(exportData);
    if (result.status === 'ok') ModalSystem.alert(result.message);
    else if (result.status !== 'cancelled') showOperationError(result, 'Could not export JSON.');
}

function debounce(func, timeout = 300) { let timer; return (...args) => { clearTimeout(timer); timer = setTimeout(() => { func.apply(this, args); }, timeout); }; }
const debouncedSimulation = debounce(() => { runSimulation(); }, 200);

const commandHistory = new CommandManager();
function executeCommand(command) { commandHistory.execute(command); setModified(true); }

document.addEventListener('keydown', (e) => {
    if ((e.ctrlKey || e.metaKey) && (e.key === 'z' || e.key === 'Z')) { if (commandHistory.undo()) setModified(true); }
    if ((e.ctrlKey || e.metaKey) && (e.key === 'y' || e.key === 'Y')) { if (commandHistory.redo()) setModified(true); }
    if ((e.ctrlKey || e.metaKey) && (e.key === 's' || e.key === 'S')) { e.preventDefault(); saveProject(false); }
});
...
function refreshAll() {
    ...
    DM.getEntities().forEach((ent, idx) => {
        UI.renderEntityCard(ent, idx, dom.entCont, {
            ...
            onCommit: (key, oldVal, newVal) => { if(oldVal!==newVal) executeCommand(new SetPropertyCommand(ent, key, oldVal, newVal, updateUI)); },
            onStatCommit: (statObj, type, oldVal, newVal) => { if(oldVal!==newVal) executeCommand(new SetPropertyCommand(statObj, type, oldVal, newVal, updateUI)); },
            onLock: () => { ent.isLocked = !ent.isLocked; refreshAll(); },
            onDelete: (i) => executeCommand(new RemoveFromCollectionCommand(DM.getEntities(), i, () => { refreshAll(); runSimulation(); })),
            onSolver: (statName) => {
    ...
    DM.getItems().forEach((item, idx) => {
        UI.renderItemCard(item, idx, dom.itemCont, {
            ...
            onInput: () => debouncedSimulation(),
            onUpdate: updateUI,
            onNameCommit: (oldVal, newVal) => { if(oldVal !== newVal) executeCommand(new SetPropertyCommand(item, 'name', oldVal, newVal, updateUI)); },
            onDelete: (i) => executeCommand(new RemoveFromCollectionCommand(DM.getItems(), i, () => { refreshAll(); runSimulation(); })),
            onModAdd: () => { executeCommand(new AppendToCollectionCommand(item.modifiers, { stat: DM.getRules().stats[0], op:'add', val:0, when: "" }, updateUI)); },
            onModDelete: (modIdx) => { executeCommand(new RemoveFromCollectionCommand(item.modifiers, modIdx, updateUI)); },
            onModCommit: (mod, key, oldVal, newVal) => { if(oldVal !== newVal) executeCommand(new SetPropertyCommand(mod, key, oldVal, newVal, updateUI)); },
            onTargetCommit: (oldTargets, newTargets) => { executeCommand(new SetPropertyCommand(item, 'targets', oldTargets, newTargets, updateUI)); },
            onTraitAdd: (newTrait) => { executeCommand(new AppendToCollectionCommand(item.traits, newTrait, updateUI)); },
            onTraitDelete: (traitIdx) => { executeCommand(new RemoveFromCollectionCommand(item.traits, traitIdx, updateUI)); },
            onTraitCommit: (traitIdx, oldTrait, newTrait) => { executeCommand(new ReplaceAtIndexCommand(item.traits, traitIdx, oldTrait, newTrait, updateUI)); }
        });
    });
}
...
document.getElementById('runSolverBtn').addEventListener('click', () => {
    ...
    if (resultGrowth !== null) {
        ...
        executeCommand(new SetPropertyCommand(
...
function analyzeBalanceIssues(rawData, maxLevel, rules) {
    ...
    entities.forEach(ent => {
        ...
        const entityLabel = `<b style="color:${safeColor(ent.color)}">${escapeHtml(ent.name)}</b>`;
        if (opRatio > 50) issues.push({ type: 'CRITICAL', percent: opRatio, msg: `${entityLabel} is OP (Overpowered) in ${opRatio.toFixed(0)}% of levels.` });
        else if (opRatio > 20) issues.push({ type: 'WARNING', percent: opRatio, msg: `${entityLabel} is strong in ${opRatio.toFixed(0)}% of levels.` });
        if (upRatio > 50) issues.push({ type: 'WEAK', percent: upRatio, msg: `${entityLabel} is too weak in ${upRatio.toFixed(0)}% of levels.` });
    });
    ...
    phases.forEach(phase => {
        ...
        if (bestEnt) issues.push({ type: 'INFO', percent: 0, msg: `[${phase.name} Game] <b style="color:${safeColor(bestEnt.color)}">${escapeHtml(bestEnt.name)}</b> dominates this phase.` });
    });
    ...
    if (crossovers.length > 0) { html += `<div style="font-size:0.85em; color:#aaa; margin-bottom:5px;">⚡ Power Spikes (Crossovers)</div>`; crossovers.forEach(c => { html += `<div class="log-item"><span class="log-level">Lv.${c.lv-1}→${c.lv}</span>: <b style="color:${safeColor(c.wColor)}">${escapeHtml(c.winnerName)}</b> overtakes <b style="color:${safeColor(c.lColor)}">${escapeHtml(c.loserName)}</b></div>`; }); }
...
function runSimulation() {
    ...
    DM.getEntities().forEach(ent => {
        const data = []; for(let lv=1; lv<=max; lv++) { const stats = Sim.getStatsAtLevel(ent, lv, DM.getItems(), rules); let val = 0; try { if (metric === 'cp') val = Sim.calculateValue(formula, stats); else val = Sim.calculateValue(formula, { a: stats, b: dummyTarget }); } catch (err) { formulaErrors.add(err.message); val = 0; } data.push(val); }
    ...
    if (hasSnapshot) {
        ...
        if (snapshot) {
            const snapData = snapshot.data; const snapRules = snapData.gameRules || snapData.rules || rules; const snapFormula = metric === 'cp' ? snapRules.cpFormula : snapRules.dmgFormula; const snapItems = snapData.items || [];
            (snapData.entities || []).forEach(ent => {
                const data = []; for(let lv=1; lv<=max; lv++) { const stats = Sim.getStatsAtLevel(ent, lv, snapItems, snapRules); let val = 0; try { if (metric === 'cp') val = Sim.calculateValue(snapFormula, stats); else val = Sim.calculateValue(snapFormula, { a: stats, b: dummyTarget }); } catch (err) { formulaErrors.add(`Snapshot: ${err.message}`); val = 0; } data.push(val); }
    ...
    let logHTML = formulaErrors.size > 0
        ? `<div class="log-item" style="color:#e74c3c;">Formula error: ${escapeHtml(Array.from(formulaErrors).join('; '))}</div>`
        : '';
    
    if (hasSnapshot) {
        const snapshot = DM.getSnapshots()[comparisonSnapshotIndex];
        const snapName = snapshot ? snapshot.name : "Snapshot";
        logHTML += `<div style="margin-bottom:15px; border-bottom:1px solid #444; padding-bottom:10px;"><div style="font-weight:bold; color:#ccc; margin-bottom:5px;">⚡ VS [${escapeHtml(snapName)}] Comparison</div>`;
        DM.getEntities().forEach(ent => {
            ...
            if (snapshot && snapshot.data && snapshot.data.entities) {
                ...
                if (snapEnt) {
                    foundInSnap = true;
                    const snapItems = snapshot.data.items || []; const snapRules = snapshot.data.gameRules || snapshot.data.rules || rules; const snapFormula = metric === 'cp' ? snapRules.cpFormula : snapRules.dmgFormula;
                    for(let lv=1; lv<=max; lv++) {
                        try { const sStats = Sim.getStatsAtLevel(snapEnt, lv, snapItems, snapRules); let val = 0; if (metric === 'cp') val = Sim.calculateValue(snapFormula, sStats); else val = Sim.calculateValue(snapFormula, { a: sStats, b: dummyTarget }); snapVal += val; } catch(e) { formulaErrors.add(`Snapshot: ${e.message}`); }
                    }
                }
            }
            if (foundInSnap) {
                ...
                logHTML += `<div style="background:#252526; border-left: 3px solid ${safeColor(ent.color)}; padding:8px; margin-top:5px; border-radius:4px; font-size:0.9em; display:flex; justify-content:space-between; align-items:center;"><span style="font-weight:bold; color:#eee;">${escapeHtml(ent.name)}</span><div style="text-align:right;"><div style="font-size:0.8em; color:#888;">Avg ${escapeHtml(metric.toUpperCase())}</div><span style="color:${color}; font-weight:bold;">${Math.round(snapVal/max)} <span style="font-size:0.8em; color:#666;">→</span> ${Math.round(currVal/max)}</span><span style="margin-left:8px; color:${color}; background:${color}20; padding:2px 6px; border-radius:4px; font-size:0.85em;">${icon} ${sign}${pcent}%</span></div></div>`;
            } else {
                logHTML += `<div style="background:#252526; border-left: 3px solid ${safeColor(ent.color)}; padding:8px; margin-top:5px; border-radius:4px; font-size:0.9em; color:#888;"><span style="font-weight:bold; color:#eee;">${escapeHtml(ent.name)}</span><span style="float:right; font-size:0.8em; color:#5fabff;">(New Entity)</span></div>`;
            }
        });
        logHTML += `</div>`;
    }
...
function renderBattleLog(allResults, heroName) {
    ...
    allResults.forEach((item, index) => {
        ...
        header.innerHTML = `<span>VS ${escapeHtml(opponent.name)}</span><div style="display:flex; gap:10px; align-items:center;"><span style="font-size:0.8em; color:#ccc;">Avg Turns: <span style="color:#fee75c;">${avgTurns}</span></span><span class="win-tag ${tagClass}">Win ${winRate}%</span></div>`;
        ...
        const updateLogDisplay = () => {
            ...
            if (logs && logs.length > 0) {
                ...
                logs.forEach(log => {
                    ...
                    let msg = ''; if (log.action === 'attack') msg = `<span class="${actorClass}">${escapeHtml(log.actor)}</span> <span style="color:#aaa;">attacked</span> <span class="log-target">${escapeHtml(log.target)}</span> → <span class="${valClass}">-${Number(log.val) || 0} HP</span>`; else if (log.action === 'die') msg = `<span class="${actorClass}">${escapeHtml(log.actor)}</span> <span style="color:#888;">${escapeHtml(log.msg)}</span>`; else if (log.action === 'miss') msg = `<span class="${actorClass}">${escapeHtml(log.actor)}</span>: <span style="color:#aaa;">Missed!</span>`; else msg = `<span class="${actorClass}">${escapeHtml(log.actor)}</span>: ${escapeHtml(log.msg)}`;
...
function renderBattleStats(attacker, statsA, allResults) {
    ...
    const createCard = (name, color, stats) => { let rows = ''; for (const [key, val] of Object.entries(stats)) { if (typeof val === 'number' && Number.isFinite(val)) { const displayVal = Number.isInteger(val) ? val : val.toFixed(2); rows += `<div class="stat-row"><span class="stat-name">${escapeHtml(key.toUpperCase())}</span><span class="stat-val">${displayVal}</span></div>`; } } return `<div class="stat-card" style="border-left-color: ${safeColor(color)}"><div class="stat-card-header">${escapeHtml(name)}</div><div class="stat-grid">${rows}</div></div>`; };
...
function initProject() {
    ...
    if (!DM.hasProjectData()) {
        ...
        const chickenId = createId();
        const godzillaId = createId();
...
function renderSnapshots() {
    ...
    snapshots.forEach((snap, idx) => {
        const item = document.createElement('div'); item.className = 'snapshot-item'; item.innerHTML = `<div class="snapshot-info"><span class="snapshot-name">${escapeHtml(snap.name)}</span><span class="snapshot-date">${escapeHtml(new Date(snap.date).toLocaleString())}</span></div><div class="snapshot-actions"><button class="load-btn" data-idx="${idx}">Load</button><button class="del-btn" data-idx="${idx}">✕</button></div>`;
        item.querySelector('.load-btn').addEventListener('click', (e) => { e.stopPropagation(); ModalSystem.confirm(`Load "${snap.name}"?`, () => { DM.loadSnapshot(idx); commandHistory.clear(); comparisonSnapshotIndex = -1; refreshAll(); runSimulation(); snapshotModal.style.display='none'; }); });
...
function renderItemSets() {
    ...
    sets.forEach((set, idx) => {
        const el = document.createElement('div'); el.className = 'snapshot-item'; el.innerHTML = `<div class="snapshot-info"><span class="snapshot-name">${escapeHtml(set.name)}</span><span style="font-size:0.8em; color:#777;">${set.items.length} items</span></div><div class="snapshot-actions"><button class="load-btn">Load</button><button class="del-btn">✕</button></div>`;
        el.querySelector('.load-btn').addEventListener('click', () => { ModalSystem.confirm(`Replace current items with "${set.name}"?`, () => { executeCommand(new ReplaceValueCommand({ read: DM.getItems, write: DM.setItems }, set.items, () => { refreshAll(); runSimulation(); })); itemSetModal.style.display = 'none'; }); });
...
if(saveItemSetBtn) saveItemSetBtn.addEventListener('click', () => { const name = document.getElementById('newItemSetName').value.trim(); if(!name) return ModalSystem.alert("Please enter a name."); DM.addItemSet(name); document.getElementById('newItemSetName').value = ''; renderItemSets(); saveProject(false); });
...
document.getElementById('unityBtn').addEventListener('click', () => { const zip = new JSZip(); zip.file("KalivraData.json", DM.exportForUnity()); zip.file("EntitySO.cs", CS_ENTITY_SO); zip.file("ItemSO.cs", CS_ITEM_SO); zip.file("KalivraImporter.cs", CS_IMPORTER); zip.generateAsync({type:"blob"}).then(function(content) { triggerDownload(content, "Kalivra_Unity_Export.zip"); }); });
...
document.getElementById('applyBulkBtn').addEventListener('click', () => { if (selectedEntityIds.length === 0) return ModalSystem.alert("Select entities first!"); const stat = document.getElementById('bulkStatSelect').value; const op = document.getElementById('bulkOpSelect').value; const val = parseFloat(document.getElementById('bulkValueInput').value) || 0; executeCommand(new BulkStatChangeCommand(DM.getEntities(), selectedEntityIds, stat, op, val, () => { refreshGrid(); refreshAll(); runSimulation(); })); const btn = document.getElementById('applyBulkBtn'); const originalText = btn.innerText; btn.innerText = "Applied!"; setTimeout(() => btn.innerText = originalText, 1000); });
...
document.getElementById('addBtn').addEventListener('click', () => { const color = '#' + Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0'); const rules = DM.getRules(); const defaults = rules.defaultValues || {}; const newStats = {}; rules.stats.forEach(s => { newStats[s] = defaults[s] ? { ...defaults[s] } : { b: 0, g: 0 }; }); executeCommand(new AppendToCollectionCommand(DM.getEntities(), { id: createId(), name: 'New Unit', color, stats: newStats, variance: 0, isLocked: false }, () => { refreshAll(); runSimulation(); })); });
document.getElementById('addItemBtn').addEventListener('click', () => { executeCommand(new AppendToCollectionCommand(DM.getItems(), { id: createId(), name: 'New Item', active: true, targets: DM.getEntities().map(e=>e.id), modifiers: [{ stat: DM.getRules().stats[0], op: "add", val: 10, when: "" }], traits: [] }, () => { refreshAll(); runSimulation(); })); });
['min','max'].forEach(action => { const btn = document.getElementById(action+'Btn'); if(btn) btn.addEventListener('click', () => action === 'min' ? Ipc.minimizeWindow() : Ipc.maximizeWindow()); });
document.getElementById('closeBtn').addEventListener('click', () => { if (isProjectModified) { ModalSystem.showExit(async () => { if (await saveProject(false)) await Ipc.closeWindow(); }, () => { Ipc.closeWindow(); }); } else { Ipc.closeWindow(); } });
...
document.getElementById('runBattleBtn').addEventListener('click', () => {
    ...
    setTimeout(() => {
        try {
            ...
            dom.battleLog.innerHTML = `<div style="padding:10px; text-align:center; color:#e74c3c;"><strong>Simulation Error!</strong><br>${escapeHtml(err.message)}</div>`;
