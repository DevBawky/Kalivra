...
현재 Node 내장 test runner 기반의 테스트가 있으므로 관련 fixture와 `npm test`를 함께
갱신한다. lint가 도입되기 전까지는 변경한 JavaScript의 `node --check`와 renderer build를
병행한다. 실행할 수 없었던 GUI·플랫폼 검증은 명시한다.
